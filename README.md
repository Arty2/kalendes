...
Appearance is two independent axes: a **flavor** (the colour palette) and a **scheme** (Auto / Light / Dark — Auto follows the system). Both also offer **Shake**, which picks at random: a fresh choice each launch, and another every time you shake the device. Set just one axis to Shake to keep the other pinned. Shaking needs a device with an accelerometer, so it works on phones and tablets (including installed Chrome/Edge web apps and iOS home-screen web apps, where iOS asks for motion access on your next tap) but not on most desktops — and never in desktop Firefox, which doesn't expose motion data to web pages at all. Where shaking isn't available, Shake still re-rolls on every launch. Each flavor defines four tokens, in a light and a dark variant: `--paper-color` (the page), `--ink-color` (text and lines), `--accent-color`, and `--link-color`; every other colour in the UI derives from those. Each flavor pairs its accent with a contrasting link colour — often another part of the namesake plant (pepper's red with a pimento green, juniper's green needle with a blue-violet berry), sometimes just a complementary hue (sage's violet flower with a deep blue, cinnamon's brick red with a deep pink). Links rest at the ink colour and reveal `--link-color` on hover/active/focus. In the swatches below the square is the paper and the ⁂ stars are the ink, accent, and link colours. Each swatch links to its SVG source in [`docs/flavors/`](docs/flavors), where the shapes carry the token names as ids.
